...
class ArkAnalyzerRunner:
    def run(self, repo_path: Path, artifact_dir: Path) -> ParserRunResult:
        ...
        if resolved is None:
            return ParserRunResult(
                ...
                manifest={
                    "parser_mode": "placeholder",
                    "reason": "arkanalyzer_unavailable",
                    "output_dir": str(output_dir),
                },
                logs=["ArkAnalyzer 不可用，回退到占位解析器。"],
                setup_hints=self._build_setup_hints(),
            )
    ...
    def _run_with_fallback(
        ...
        try:
            ...
            return ParserRunResult(
                ...
                manifest={
                    "parser_mode": "placeholder",
                    "reason": "arkanalyzer_failed",
                    "output_dir": str(output_dir),
                    "stdout": exc.stdout or "",
                    "stderr": exc.stderr or "",
                },
                logs=["ArkAnalyzer 执行失败，回退到占位解析器。"],
                setup_hints=self._build_setup_hints(exc.stderr or exc.stdout or str(exc)),
            )
            ...
            return ParserRunResult(
                ...
                manifest={
                    "parser_mode": "placeholder",
                    "reason": "arkanalyzer_no_output",
                    "output_dir": str(output_dir),
                },
                logs=["ArkAnalyzer 未产生可解析工件，回退到占位解析器。"],
                setup_hints=[str(exc)],
            )
    ...
    def _build_setup_hints(self, error_message: str = "") -> list[str]:
        hints: list[str] = []
        if not ARKANALYZER_ROOT.exists():
